...
class ResourceChange(object):
    ...
    def __init__(self, o: typing.Mapping):
        ...
        self.physical_resource_id = o.get(
            "PhysicalResourceId"
        )  # type: typing.Optional[str]
        self.replacement = (
            Replacement(o["Replacement"]) if "Replacement" in o else None
        )  # type: typing.Optional[Replacement]
        self.resource_type = o["ResourceType"]  # type: str
        self.scope = [Scope(x) for x in o["Scope"]]

        # Objects
        self.details = [
            ResourceChangeDetail(x, self.logical_resource_id) for x in o["Details"]
        ]
    ...
    def render(self, dot: Dot) -> None:
        ...
        label_properties = dict(
            resource_type=self.resource_type, action=self.action.value,
        )
        if self.replacement:
